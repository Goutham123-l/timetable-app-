...
function Subjects() {
  const [items, setItems] = useState<any[]>([]);
  const [form, setForm] = useState({ name: "", code: "", type: "THEORY", weeklyHours: 3, year: "" as number | "" });
  const [editingId, setEditingId] = useState<number | null>(null);
  ...
  function startEdit(s: any) {
    setEditingId(s.id);
    setForm({ name: s.name, code: s.code, type: s.type, weeklyHours: s.weeklyHours, year: s.year || "" });
  }
  function resetForm() {
    setEditingId(null);
    setForm({ name: "", code: "", type: "THEORY", weeklyHours: 3, year: "" });
  }
